...
export class AdminNotificationManagementComponent implements OnInit {
  ...
  fetchUsersLastNotifications() {
    this.users.forEach((user) => {
      this.notificationService.getLastNotification(user._id).subscribe(
        (notification: Notification | null) => {
          user.lastNotification = notification;
        },
        (error) => {
          console.error('Error fetching last notification for user:', error);
        }
      );
    });
  }
